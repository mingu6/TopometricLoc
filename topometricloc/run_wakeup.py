...
def trial_start_locations(n_query, n_steps, n_trials):
    # TODO: decide on random or uniform spacing
    ...
    start_inds = rng.choice(np.arange(max_start_ind), replace=False, size=n_trials)
    return start_inds
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--descriptor", type=str, default="hfnet",
                        help="global descriptor to evaluate on", choices=["hfnet", "netvlad_fullres", "netvlad_640x480", "netvlad_prior"])
    parser.add_argument("-w", "--width", type=int, default=10,
                    help="maximum distance for possible transition between nodes")
    parser.add_argument("-m", "--methods", nargs="+", type=str,
                        default=["baseline", "xu20topo", "stenborg20", "topometric", "nooff"])
    ...
    for query in pbarq:
        ...
        for method in pbarm:
            ...
            if method == 'baseline':
                n_steps = 1  # localize first image only
            else:
                n_steps = int(args.max_dist / float(args.query_filename.split('_')[1]))
            ...
            for trial, start_ind in enumerate(tqdm(start_indices, desc="trials", leave=False)):
                ...
                for t in range(n_steps):
                    ...
                    pose_est, scores[trial, t] = loc.check_convergence()
